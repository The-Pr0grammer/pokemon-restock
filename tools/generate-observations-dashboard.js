...
function buildVisualSummary({ observations, candidates, sourceStatuses, marketEstimates, generatedAt }) {
  ...
  const signals = candidates.map(candidate => ({
    name: candidate.name || 'Unnamed product', tier: candidate.candidate_type || candidate.status || 'unknown', retail_price: candidate.retail?.price ?? null, market_estimate: candidate.market?.estimate ?? null, net_profit: candidate.math?.estimated_net_profit ?? null, roi_pct: candidate.math?.roi_pct ?? null, source: candidate.retail?.source || 'unknown', confidence: candidate.confidence || 'unknown', evidence_count: candidate.market?.evidence_count ?? candidate.market?.evidence?.length ?? 0, price_history: Array.isArray(candidate.market?.price_history) ? candidate.market.price_history : [], price_change: candidate.market?.price_change ?? candidate.market?.trend?.price_change ?? null, trend: candidate.market?.trend ?? null,
  }));
  return { generated_at: generatedAt, source_health: sourceHealth, funnel: { observed: observations.length, verified: observations.filter(obs => obs.confidence === 'verified').length, actionable: observations.filter(obs => obs.procurement_eligible === undefined ? obs.availability === 'in_stock' : obs.procurement_eligible === true).length, enriched, investigate: signals.filter(signal => signal.tier === 'investigate').length, opportunities: signals.filter(signal => signal.tier === 'flip_candidate').length }, signals };
}
...
function renderCandidateRows(candidates) {
  return candidates.map(candidate => `<tr><td class="product"><a href="${escapeHtml(candidate.retail?.url)}">${escapeHtml(candidate.name || 'Unnamed product')}</a></td><td>${escapeHtml(formatPrice({ price: candidate.retail?.price, currency: candidate.retail?.currency }))}</td><td>${escapeHtml(formatPrice({ price: candidate.market?.estimate, currency: candidate.market?.currency }))}</td><td>${escapeHtml(formatPrice({ price: candidate.math?.estimated_net_profit, currency: candidate.retail?.currency }))}</td><td>${escapeHtml(candidate.math?.roi_pct ?? 'N/A')}%</td><td>${escapeHtml(candidate.market?.source || 'unknown')} (${escapeHtml(candidate.market?.evidence_count ?? 0)} evidence)</td><td>${escapeHtml(candidate.candidate_type || candidate.confidence || 'unknown')}</td></tr>`).join('');
}
